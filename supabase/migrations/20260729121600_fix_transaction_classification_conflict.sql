-- Recompile the classification RPC with an explicit PL/pgSQL name-resolution
-- policy. The function returns a "context" column and also references the
-- merchant_rules.context column in an ON CONFLICT target.
